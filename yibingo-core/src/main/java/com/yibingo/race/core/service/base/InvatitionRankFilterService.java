...
public class InvatitionRankFilterService {

    public static List<InvatitionRank> getListByFilter(InvatitionRankBaseService invatitionRankBaseService, InvatitionRankFilterMapper filterMapper) {

        QueryWrapper<InvatitionRank> wrapper = new QueryWrapper<>();
        ...
        if (filterMapper.userIdIn != null) wrapper.in("user_id", filterMapper.userIdIn);
        ...
        Page<InvatitionRank> markPage = new Page<>(page, row);

        Page<InvatitionRank> resultList = invatitionRankBaseService.page(markPage, wrapper);
